...
import { Avatar, Button, EmptyState, CopyButton, Toggle } from '../components/ui/index'
import {
  IlluNotFound,
  IlluAccessDenied,
  IlluEmptyPosts,
...
export default function AdminBoardPage({ params }) {
  const {
    getBoardBySlug, getBoardPosts, currentUser,
    updatePost, deletePost, updateBoard, deleteBoard, loadBoardPosts,
  } = useApp()
  ...
  if (!board) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center pt-14">
        <EmptyState illustration={<IlluNotFound size={96} />}
  ...
  if (!currentUser || currentUser.id !== board.ownerId) {
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center pt-14">
        <EmptyState illustration={<IlluAccessDenied size={96} />}
          title="Access denied"
          body="You don't have permission to manage this board."
          action={<Button onClick={() => navigate('/dashboard')}>My dashboard</Button>}
        />
      </div>
    )
  }
  ...
  const filtered = allPosts.filter(p => tagFilter === 'all' || p.tag === tagFilter)
  ...
  const handleSaveSettings = () => {
    updateBoard(board.id, { settings: settingsForm })
    setSaved(true)
    ...
    <div className="min-h-screen bg-[#FAFAFA] pt-14 flex flex-col">
      ...
      <div className={`bg-gradient-to-br ${board.headerGradient ?? 'from-teal-600 to-emerald-500'}`}>
        <div className="max-w-5xl mx-auto px-6 py-8">
          ...
          <button onClick={() => navigate('/dashboard')}
            ...
            <Icons.ArrowLeft size={13} className="group-hover:-translate-x-0.5 transition-transform" />
            Dashboard
          </button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-2.5 mb-1.5">
                <h1 className="text-[24px] sm:text-[28px] font-extrabold text-white"
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <CopyButton text={boardUrl} className="bg-white/10 text-white border border-white/20 hover:bg-white/20" />
              <Button size="sm"
              ...
              <Button size="sm" onClick={() => setAddOpen(true)} leftIcon={<Icons.Plus size={13} />}>
                Add request
              </Button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4 mt-6 pt-5 border-t border-white/10">
            {[
              ...
              { label: 'Total votes',  value: allPosts.reduce((s, p) => s + p.upvotes, 0) },
              { label: 'Interactions', value: `${usedCount}/${isPro ? '∞' : '25'}` },
              { label: 'Shipped',      value: allPosts.filter(p => p.status === 'live').length },
            ].map(s => (
              <div key={s.label}>
                <p className="text-[22px] font-extrabold text-white leading-none drop-shadow">{s.value}</p>
      ...
      <div className="sticky top-14 z-30 bg-white border-b border-[#F3F4F6] shadow-sm">
        <div className="max-w-5xl mx-auto px-6 h-11 flex items-center gap-1">
          ...
          <span className="text-[11px] text-[#9CA3AF] font-medium hidden sm:block">Admin view</span>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-5xl w-full mx-auto px-6 py-8">
          ...
          <div className="flex gap-8">
            <aside className="hidden lg:block w-44 flex-shrink-0">
              <div className="sticky top-28 flex flex-col gap-5">
                <FilterSection label="Sort">
                  <FilterChip active={sort === 'top'} onClick={() => setSort('top')}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[12px] text-[#9CA3AF]">{sorted.length} request{sorted.length !== 1 ? 's' : ''}</p>
              ...
              {sorted.length === 0 ? (
                <EmptyState illustration={<IlluEmptyPosts size={88} />}
                  title="No requests yet"
                  body="Requests from your users will appear here."
                  action={<Button onClick={() => setAddOpen(true)} leftIcon={<Icons.Plus size={13} />}>Add first request</Button>}
                />
              ) : (
                <div className="flex flex-col gap-3">
                  {sorted.map(post => (
                    <FeedbackCard key={post.id} post={post} board={board} adminMode
                      onStatusChange={handleStatusChange} onDelete={handleDelete} onPin={handlePin} />
                  ))}
                </div>
              )}
            </div>
        ...
        {tab === 'roadmap' && (
          <RoadmapView board={board} posts={allPosts} adminMode
            onStatusChange={handleStatusChange} onDelete={handleDelete} onPin={handlePin} />
        )}

        {tab === 'settings' && (
          <div className="max-w-xl flex flex-col gap-5">
            <SettingsCard title="Consumer identity" subtitle="Control what information submitters must provide">
              <Toggle checked={settingsForm.requireName}
                onChange={v => setSettingsForm(f => ({ ...f, requireName: v, ...(v ? { allowAnonymous: false } : {}) }))}
                label="Require submitter's name" hint="Consumers must enter their name to submit or vote" />
              <Toggle checked={settingsForm.requireEmail}
                onChange={v => setSettingsForm(f => ({ ...f, requireEmail: v, ...(v ? { allowAnonymous: false } : {}) }))}
                label="Require email address" hint="Email is collected privately — never shown publicly" />
              <Toggle checked={settingsForm.allowAnonymous}
                onChange={v => setSettingsForm(f => ({ ...f, allowAnonymous: v, ...(v ? { requireName: false, requireEmail: false } : {}) }))}
                label="Allow fully anonymous submissions" hint="No name or email required" />
              <Toggle checked={settingsForm.showVoterCount}
                onChange={v => setSettingsForm(f => ({ ...f, showVoterCount: v }))}
                label="Show vote counts publicly" />
            </SettingsCard>

            <SettingsCard title="Share your board" subtitle="Send this link to your users">
              <div className="flex items-center gap-2 bg-[#F9FAFB] rounded-xl px-4 py-3 border border-[#F3F4F6]">
                <code className="flex-1 text-[12px] text-[#374151] break-all">{boardUrl}</code>
              ...
              <div className={`flex items-center gap-2 text-[12px] px-3 py-2 rounded-lg mt-2
                ...
                {board.visibility === 'private' ? <IlluPrivate size={14} /> : <IlluPublic size={14} />}
                {board.visibility === 'private'
            ...
            {!isPro && (
              <div className="bg-gradient-to-r from-violet-50 to-purple-50 border border-violet-200 rounded-2xl p-5">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[13px] font-bold text-violet-800 mb-1">Free plan</p>
                    <p className="text-[12px] text-violet-600">{usedCount}/25 interactions used. Upgrade for unlimited interactions, private boards, and more.</p>
                  </div>
                  <Button variant="pro" size="sm" className="flex-shrink-0"
                    onClick={() => navigate('/dashboard')} leftIcon={<Icons.Crown size={13} />}>
                    Upgrade
                  </Button>
                </div>
                <div className="mt-3 h-1.5 bg-violet-200 rounded-full overflow-hidden">
                  <div className="h-full bg-violet-500 rounded-full transition-all"
                    style={{ width: `${Math.min(100, (usedCount / 25) * 100)}%` }} />
                </div>
              </div>
            )}

            <Button onClick={handleSaveSettings} disabled={saved}>
              {saved ? <><Icons.Check size={14} /> Settings saved</> : 'Save settings'}
            </Button>

            <div className="bg-rose-50 border border-rose-100 rounded-2xl p-5 mt-2">
              <h3 className="text-[14px] font-bold text-rose-800 mb-1">Danger zone</h3>
              <p className="text-[12px] text-rose-400 mb-3">Permanently deletes this board and all its requests.</p>
              <Button variant="danger" leftIcon={<Icons.Trash size={13} />}
                onClick={() => {
                  if (confirm(`Delete "${board.name}" and all its requests? This cannot be undone.`)) {
