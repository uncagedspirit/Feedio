...
const LS = {
  USER: "feedio_user",
  BOARDS: "feedio_boards",
  POSTS: "feedio_posts",
  VOTES: "feedio_votes",
  FP: "feedio_fingerprint",
};
const ls_get = (k, fb) => {
  try {
...
// ─── GRADIENT LOOKUP ─────────────────────────────────────────────────────────
const GRADIENT_MAP = {
  "#14B8A6": "from-teal-600 to-emerald-500",
  "#8B5CF6": "from-violet-600 to-purple-500",
  "#F97316": "from-orange-500 to-amber-400",
  "#EF4444": "from-rose-500 to-red-400",
  "#3B82F6": "from-blue-600 to-sky-500",
  "#EC4899": "from-pink-500 to-rose-400",
  "#22C55E": "from-green-500 to-emerald-400",
  "#EAB308": "from-yellow-500 to-amber-400",
};
...
const dbBoard = (r) => ({
  ...
  headerGradient: r.header_gradient,
  visibility: r.visibility,
  ownerId: r.owner_id,
  ownerName: r.owner_name,
  ownerAvatarInitials: r.owner_avatar_initials,
...
async function callEnforceAccess() {
  try {
    ...
    if (error) {
      console.warn(
        "[enforce-access] edge function returned error:",
        error.message ?? error,
      );
      return null;
    }
    ...
    console.warn(
      "[enforce-access] network error — skipping (user remains logged in):",
      err,
    );
    return null;
  }
...
async function invokeRedeemTrial(token) {
  try {
    ...
    if (error) {
      console.warn(
        "[redeem-trial] edge function returned error:",
        error.message ?? error,
      );
      return { data: null };
    }
...
export function AppProvider({ children }) {
  ...
  const [boards, setBoards] = useState(() =>
    SUPABASE_ENABLED ? [] : ls_get(LS.BOARDS, MOCK_BOARDS),
  );
  const [posts, setPosts] = useState(() =>
    SUPABASE_ENABLED ? [] : ls_get(LS.POSTS, MOCK_POSTS),
  );
  const [myVotes, setMyVotes] = useState(() => new Set(ls_get(LS.VOTES, [])));
  const fp = useRef(getFingerprint());

  useEffect(() => {
    if (!SUPABASE_ENABLED) ls_set(LS.BOARDS, boards);
  }, [boards]);
  useEffect(() => {
    if (!SUPABASE_ENABLED) ls_set(LS.POSTS, posts);
  }, [posts]);
  useEffect(() => {
    ls_set(LS.VOTES, [...myVotes]);
  }, [myVotes]);
  ...
  const upgradePlan = useCallback(async () => {
    if (!SUPABASE_ENABLED) {
      ...
      setCurrentUser((u) =>
        u ? { ...u, plan: "pro", subscriptionState: "active" } : u,
      );
  ...
  useEffect(() => {
    if (SUPABASE_ENABLED) loadBoards();
  }, [loadBoards]);

  const getBoardBySlug = useCallback(
    (slug) => boards.find((b) => b.slug === slug) ?? null,
    [boards],
  );
  ...
  const getUserBoards = useCallback(
    (uid) => boards.filter((b) => b.ownerId === uid),
    [boards],
  );
  ...
  const createBoard = useCallback(async (data) => {
    ...
    const row = {
      ...
      accent_color: data.accentColor ?? "#14B8A6",
      header_gradient: toGradient(data.accentColor),
      visibility: data.visibility ?? "public",
      owner_id: data.ownerId,
      owner_name: data.ownerName,
      owner_avatar_initials: data.ownerAvatarInitials,
      owner_avatar_color: data.ownerAvatarColor,
      tags: data.tags ?? ["Feature", "Bug", "Other"],
      settings: {
        requireName: data.settings?.requireName ?? true,
        requireEmail: data.settings?.requireEmail ?? false,
        allowAnonymous: data.settings?.allowAnonymous ?? false,
        showVoterCount: data.settings?.showVoterCount ?? true,
      },
  ...
  const updateBoard = useCallback(
    async (boardId, updates) => {
      ...
      if (updates.totalInteractions != null)
        row.total_interactions = updates.totalInteractions;
      ...
      const { error } = await supabase
        ...
        .eq("id", boardId);
      if (error) {
        console.error("[boards] update:", error);
        loadBoards();
      }
    },
    [loadBoards],
  ...
  const getBoardPosts = useCallback(
    (boardId) => posts.filter((p) => p.boardId === boardId),
    [posts],
  );
    ...
    async (boardId, data) => {
      ...
      if (!SUPABASE_ENABLED) {
        ...
        setBoards((prev) =>
          prev.map((b) =>
            b.id === boardId
              ? { ...b, totalInteractions: (b.totalInteractions ?? 0) + 1 }
              : b,
          ),
        );
        const boardForAnalytics = boards.find((b) => b.id === boardId);
        if (boardForAnalytics)
          Analytics.postSubmitted(
            boardForAnalytics,
            dbPost({ ...row, id: `post-${Date.now()}` }),
          );
        return post;
      }
      const { data: ins, error } = await supabase
      ...
      await supabase
        .from("boards")
        .update({
          total_interactions: (getBoardBySlug("") || {}).totalInteractions,
        })
        .eq("id", boardId);
      setBoards((prev) =>
        prev.map((b) =>
  ...
  const loadMyVotes = useCallback(async (boardId) => {
    ...
    if (data?.length)
      setMyVotes((prev) => {
        ...
        data.forEach((r) => next.add(r.post_id));
        return next;
      });
  }, []);
  ...
  const toggleUpvote = useCallback(
    async (postId, boardId) => {
      const voted = myVotes.has(postId);
      const delta = voted ? -1 : 1;
      setMyVotes((prev) => {
        const next = new Set(prev);
        voted ? next.delete(postId) : next.add(postId);
        return next;
      });
      setPosts((prev) =>
        prev.map((p) =>
  ...
  const canInteract = useCallback(
    (board) => {
      ...
      let ownerPlan = "free";
      if (currentUser?.id === board.ownerId)
        ownerPlan = currentUser.plan ?? "free";
      else
        ownerPlan =
          MOCK_USERS.find((u) => u.id === board.ownerId)?.plan ?? "free";
