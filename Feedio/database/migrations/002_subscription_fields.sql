...
BEGIN
  SELECT jsonb_build_object(
    ...
    'active_subs',       (SELECT COUNT(*) FROM profiles WHERE subscription_state = 'active'),
    'free_users',        (SELECT COUNT(*) FROM profiles WHERE plan = 'free'),
    'cancel_scheduled',  (SELECT COUNT(*) FROM profiles WHERE subscription_state = 'cancel_scheduled'),
    'in_grace',          (SELECT COUNT(*) FROM profiles WHERE subscription_state = 'in_grace'),
    'expired',           (SELECT COUNT(*) FROM profiles WHERE subscription_state = 'expired'),
    'trial_links_active',(SELECT COUNT(*) FROM trial_links WHERE is_active = true),
...
RETURNS TABLE (
  id uuid, name text, masked_email text, plan text,
  subscription_state text, boards_count bigint,
...
BEGIN
  ...
  SELECT
    ...
    p.plan, p.subscription_state,
    COUNT(b.id) as boards_count,
    p.created_at, p.last_active_at
  FROM profiles p
  JOIN auth.users u ON u.id = p.id
  LEFT JOIN boards b ON b.owner_id = p.id
