 ...
 *
 * Validates a magic trial link token and activates a free Pro trial
 * for the authenticated user. No payment details required.
 *
 * Deploy:
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!trialToken) return json({ error: 'token is required' }, 400)
  } catch {
    return json({ error: 'Invalid JSON body' }, 400)
  }

  // Guard: don't overwrite a real paid subscription
  ...
  if (!link) return json({ error: 'invalid_token' }, 400)

  const row = link as TrialLinkRow
  if (!row.is_active)                            return json({ error: 'invalid_token' }, 400)
  if (new Date(row.expires_at) < new Date())     return json({ error: 'token_expired' }, 400)
  if (row.use_count >= row.max_uses)             return json({ error: 'token_exhausted' }, 400)
  ...
  if (existing) {
    const endsAt = new Date(Date.now() + row.trial_days * 86400000).toISOString()
  ...
  // Record redemption + increment use_count
  ...
  const endsAt = new Date(Date.now() + row.trial_days * 86400000).toISOString()
  console.log(`[redeem-trial] trial activated: user=${userId} days=${row.trial_days} link=${row.id}`)
