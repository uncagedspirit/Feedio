...
import { IlluRocket, IlluCelebration, IlluPublic, IlluPrivate } from '../illustrations'
...
export default function CreateBoardModal({ open, onClose }) {
  ...
  // Tags
  const [tags, setTags]       = useState(['Feature', 'Bug', 'Improvement', 'Other'])
  ...
  const handleCreate = async () => {
    ...
    const board = createBoard({
      ...
      tags,
      settings: { requireName, requireEmail, allowAnonymous: allowAnon, showVoterCount },
    })
    setLoading(false)
    setCreated(board)
    setStep(2)
  ...
  const handleClose = () => {
    setStep(0); setName(''); setTagline(''); setDesc('')
    setWebsite(''); setAccent(ACCENT_COLORS[0]); setVis('public')
    setReqName(true); setReqEmail(false); setAllowAnon(false); setShowCount(true)
    setTags(['Feature', 'Bug', 'Improvement', 'Other'])
    setCreated(null); setLoading(false)
    onClose()
  }

  if (atFreeLimit && open) {
    return (
      <Modal open={open} onClose={handleClose} title="Board limit reached" maxWidth="max-w-sm">
        <div className="text-center py-2">
          <div className="flex justify-center mb-3"><IlluRocket size={80} /></div>
          <p className="text-[13px] text-[#6B7280] mb-5">
            Free plan includes <strong>1 public board</strong>. Upgrade to Pro for unlimited boards, private boards, and more.
          </p>
          <div className="flex flex-col gap-2">
            <Button variant="pro" fullWidth onClick={() => { upgradePlan(); handleClose() }}>
              <Icons.Crown size={14} /> Upgrade to Pro — $19/mo
            </Button>
            <Button variant="ghost" fullWidth onClick={handleClose}>Maybe later</Button>
          </div>
        </div>
      </Modal>
    )
  ...
  return (
    <Modal
      open={open}
      onClose={handleClose}
      title={step === 2 ? undefined : step === 0 ? 'Create a board' : 'Board settings'}
      maxWidth="max-w-lg"
    >
      {/* ── STEP 2: SUCCESS ── */}
      {step === 2 && created && (
        <div className="text-center py-4">
          <div className="flex justify-center mb-4"><IlluCelebration size={88} /></div>
          <h2 className="text-[18px] font-bold text-[#111827] mb-2">{created.name} is live!</h2>
          <p className="text-[13px] text-[#6B7280] mb-2">Share this link with your users:</p>
          <div className="flex items-center gap-2 bg-[#F3F4F6] rounded-xl px-4 py-3 mb-6 text-left">
            ...
            <button
              onClick={() => navigator.clipboard.writeText(`${window.location.origin}/boards/${created.slug}`)}
          ...
          {created.visibility === 'public' && (
            <p className="text-[12px] text-emerald-600 bg-emerald-50 px-3 py-2 rounded-lg mb-4">
              <IlluPublic size={14} className="inline mr-1" /> This board will appear in the public boards directory.
            </p>
          )}
          {created.visibility === 'private' && (
            <p className="text-[12px] text-violet-600 bg-violet-50 px-3 py-2 rounded-lg mb-4">
              <IlluPrivate size={14} className="inline mr-1" /> Only people with the link can view this board.
            </p>
          )}
          <div className="flex gap-3">
            <Button variant="outline" fullWidth onClick={handleClose}>Close</Button>
            <Button fullWidth onClick={() => { navigate(`/boards/${created.slug}`); handleClose() }}>
              View board
            </Button>
          </div>
        </div>
      ...
      {step === 0 && (
        <div className="flex flex-col gap-5">
          ...
          {/* Accent color */}
          <div>
            <p className="text-[12px] font-semibold text-[#374151] mb-2">Accent color</p>
          ...
          {/* Visibility */}
          <div>
            <p className="text-[12px] font-semibold text-[#374151] mb-2">Visibility</p>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <Button fullWidth onClick={() => setStep(1)} disabled={!name.trim()}>
              Next: Settings →
            </Button>
          </div>
        </div>
      ...
      {step === 1 && (
        <div className="flex flex-col gap-5">
          {/* Identity / privacy */}
          <div>
            <p className="text-[12px] font-semibold text-[#374151] mb-3">Consumer identity</p>
            <div className="flex flex-col gap-3 bg-[#F9FAFB] rounded-xl p-4">
              <Toggle
                checked={requireName}
                onChange={(v) => { setReqName(v); if (v) setAllowAnon(false) }}
                label="Require submitter's name"
                hint="Consumers must enter their name to submit or vote"
              />
              <Toggle
                checked={requireEmail}
                onChange={(v) => { setReqEmail(v); if (v) setAllowAnon(false) }}
                label="Require email address"
                hint="Email is collected but hidden from public view"
              />
              <Toggle
                checked={allowAnon}
                onChange={(v) => { setAllowAnon(v); if (v) { setReqName(false); setReqEmail(false) } }}
          ...
          {/* Tags */}
          <div>
            <p className="text-[12px] font-semibold text-[#374151] mb-2">Request categories / tags</p>
            <div className="flex flex-wrap gap-2 mb-2">
              {tags.map(t => (
                <span key={t} className="flex items-center gap-1 text-[11px] bg-[#F3F4F6] text-[#374151] px-2.5 py-1 rounded-full">
                  {t}
                  <button onClick={() => removeTag(t)} className="text-[#9CA3AF] hover:text-rose-500 transition-colors">
                    <Icons.X size={9} />
                  </button>
                </span>
              ))}
...
function VisBtn({ active, onClick, icon, label, hint, disabled, disabledHint }) {
  return (
    <button
    ...
    >
      <div className="flex items-center gap-2">
        <span className={active ? 'text-teal-600' : 'text-[#6B7280]'}>{icon}</span>
        <span className="text-[13px] font-semibold text-[#111827]">{label}</span>
        {disabled && (
          <span className="text-[10px] font-bold text-violet-600 bg-violet-100 px-1.5 py-0.5 rounded-full">Pro</span>
        )}
      </div>
      <p className="text-[11px] text-[#9CA3AF] pl-5">{disabled ? disabledHint : hint}</p>
    </button>
  )
}
